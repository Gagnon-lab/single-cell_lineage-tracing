...
```{r load lineage barcode data}
# load lineage barcode data
un1_data <- read.table('data/tables/unedited1_bestlineagebarcode.tsv', sep = '\t', header = T)
un2_data <- read.table('data/tables/unedited2_bestlineagebarcode.tsv', sep = '\t', header = T)
```
...
# we have a data frame of barcodes associated with unedited1 or unedited2 (un1_data and un2_data)
# let's treat these metadata frames separately and then combine them later with rbind before adding back as meta data to the Seurat object
...
```{r add suffix to cell-ids in lineage dataframe}
...
```{r join transcriptome cell ids with lineage cell ids}
# using left_join from dplyr maintains the order of the cell_ids according to the Seurat object's metadata (stored those in un_cells_1 and un_cells_2) and only uses barcodes for cells which are in the transcriptome (Seurat object)
un_cells_1_barcodes <- left_join(un_cells_1, un1_data)
un_cells_2_barcodes <- left_join(un_cells_2, un2_data)
```

```{r condition subsample identity}
# add category to differentiate sub samples
un_cells_1_barcodes$condition <- "unedited1"
un_cells_2_barcodes$condition <- "unedited2"
```

```{r get new copy of transcriptome cells}
# recopy unedited_testis@meta.data 
# don't want extra cell.id column this time
# subset for just unedited1 cells 
...
```{r subset columns for Seurat metadata integration}
# now subset the columns that are going into the Seurat object for unedited1
Cells1.info <- subset(Cells1, select = c('barcode.count', 'barcode', 'site1', 'site2','site3', 'site4','site5','site6','site7', 'site8','site9', 'site10'))
```
...
Cells2.info <- subset(Cells2, select = c('barcode.count', 'barcode', 'site1', 'site2','site3', 'site4','site5','site6','site7', 'site8','site9', 'site10'))
```
```{r combine unedited dataframes}
# stack these two data frames on top of each other
...
```{r load lineage barcode data}
# load lineage barcode data
ed1_data <- read.table('data/tables/inj_heat1_bestlineagebarcode.tsv', sep = '\t', header = T)
ed2_data <- read.table('data/tables/inj_heat2_bestlineagebarcode.tsv', sep = '\t', header = T)
```
...
# in ed1_data (lineage barcode dataframe), the cell-ids are just a string of 16 letters, but ed_meta_1 (transcriptome dataframe), they have a -1 at the end. This is because we integrated two channels of 10X to make the edited object. 
# So we'll add -1 to the cell-ids in the un1_data
...
```{r join transcriptome cell ids with lineage cell ids}
# using left_join from dplyr maintains the order of the cell_ids according to the Seurat object's metadata (stored those in ed_cells_1 and ed_cells_2) and only uses barcodes for cells which are in the transcriptome (Seurat object)
